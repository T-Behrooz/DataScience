...
def find_IDMU(df):  
    ...
    for i, col in enumerate(df.columns): 
        if 0 < i <= I:  
           IDMU[col] = df[col].min() ;print(IDMU) 
        elif i>I and i<len(df.columns):
           IDMU.update({col:df[col].max()})
    IDMU1 = pd.DataFrame([IDMU], columns=IDMU.keys()) ;print(IDMU1) 
    return IDMU1      
#--------------------------finding DMUI and DMUO sets-------------------------------------            
def filter_dmuo_dmui_sets_old(df, I, O):
    dfI = df.iloc[:, :I+1];print(dfI)
    dfO = df.iloc[:, I+1:];print(dfO)
    
    inputFilter = pd.concat([df[df[i] == dfI[i].min()] for i in range(1,len(dfI.columns))], ignore_index=False).drop_duplicates();print(inputFilter)
    
    outputFilter = pd.concat([df[df[i] == dfO[i].max()] for i in dfO.columns], ignore_index=False).drop_duplicates()
...
def filter_dmuo_dmui_sets(df, I, O):  
    ...
    dfI = df.iloc[:, :I+1]; print(dfI) 
    input_filters = []  
    for i in range(1, len(dfI.columns)):  
        input_filter = df[df[dfI.columns[i]] == dfI[dfI.columns[i]].min()]  
        input_filters.append(input_filter)  
    inputFilter = pd.concat(input_filters, ignore_index=True).drop_duplicates()  
    print(inputFilter)
    # فیلتر خروجی (DMUO)  
    dfO = df.iloc[:, I+1:] ;print(dfO) 
    output_filters = []  
    for i in dfO.columns:  
        output_filter = df[df[i] == dfO[i].max()]  
        output_filters.append(output_filter)  
    outputFilter = pd.concat(output_filters, ignore_index=True).drop_duplicates()  
...
def choice_dmui_or_dmuo_from_multiples(newdataset,I,O):
    ...
    return result
#------------------------------------------------------------------------------
def find_between_dmus(dmu1,dmu2,df,I):
    ...
    for row in df.index:
        ...
        if (row!=n1)and(row!=n2):
                temp1=[];temp2=[];temp1.clear();temp2.clear();
                temp=[];temp.clear()
                test=df.filter(items=[row],axis=0);
                for i in range(1,I+1):
