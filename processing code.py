...
for dmu in K:  
    ...
    # Model Building  
    model = LpProblem('DEA_Dominated', LpMinimize)  

    # Decision variables Building  
    alfa_r = LpVariable(f'alfa_r')  
    ...
    for i in II:  
        model += lpSum([  
            alfa_r +lambda_k[k] * -XX[i][k]  
        ] for k in KK)  >= -X[i][dmu]
          
    for j in JJ:  
        model += lpSum([  
            alfa_r +lambda_k[k] * YY[j][k]
        ] for k in KK) >= Y[j][dmu]  
...
try:  
    wb = load_workbook(f"{drive}_Dominated_{file_name}")  
    ...
    # Save the workbook  
    wb.save(f"{drive}_Dominated_{file_name}")  
