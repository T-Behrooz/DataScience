...
for dmu in KK:  
    ...
    result = {  
        ...
        'Mathematical Formulation': f"min {allfa_r}\nsubject to:\n{', '.join([f'{allfa_r} - ({lambda_k[k]})*{XX[i][k]} >= -{XX[i][dmu]}' for i in II for k in KK])}\n{', '.join([f'{allfa_r} + ({lambda_k[k]})*{YY[j][k]} >= {YY[j][dmu]}' for j in JJ for k in KK])}\n{','.join([f'{lambda_k[k]} >= 0' for k in KK])}\n{sum([lambda_k[k] for k in KK])} == 1"  
    }  
    for k in KK:  
