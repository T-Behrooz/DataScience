...
path1='d:\\Raw 1600 Dmus DataSet.xlsx'
path2='d:\\Raw 1400 Dmus DataSet ( no Zero ).xlsx'
path3="d:\\test.xlsx"
df = pd.read_excel(path2,sheet_name='data')
...
out_path1='d:\\{}Raw 1600 Dmus DataSet.xlsx'.format("@@@ Uniq inputs & outputs of __ ")
out_path2='d:\\{}Raw 1400 Dmus DataSet ( no Zero ).xlsx'.format("@@@ Uniq inputs & outputs of __ ")
try:  
...
# Populate input and output parameters واحد های تحت ارزیابی 
for i in I:  
    for k in K:  
...
# Populate input and output parameters واحد های ماکزیمم و می نی موم 
for i in II:  
    for k in KK:  
        XX[i][k] = s_Dmu.loc[s_Dmu['DMU'] == k, i].values[0]  
for j in JJ:  
    for k in KK:  
        YY[j][k] = s_Dmu.loc[s_Dmu['DMU'] == k, j].values[0]
...
for dmu in K:  
    ...
    # Model Building  
    model = LpProblem('remove dominated', LpMinimize)  

    # Decision variables Building  
    allfa_r = LpVariable(f'allfa_r', lowBound=0)  
    ...
    for i in II:  
        model += lpSum([  
           allfa_r +lambda_k[k] *-X[i][k]  
        ] for k in KK) >= - X[i][dmu]  
    for j in JJ:  
        model += lpSum([  
           allfa_r + lambda_k[k] * Y[j][k]  
        ] for k in KK) >= Y[j][dmu]  
